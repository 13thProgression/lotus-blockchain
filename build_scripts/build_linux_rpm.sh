...
if [ ! "$1" ]; then
  echo "This script requires either amd64 of arm64 as an argument"
	...
	DIR_NAME="lotus-blockchain-linux-x64"
else
	#PLATFORM="$1"
	DIR_NAME="lotus-blockchain-linux-arm64"
fi
...
if [ "$REDHAT_PLATFORM" = "x86_64" ]; then
	echo "Create LOTUS-blockchain-$LOTUS_INSTALLER_VERSION.rpm"

	# shellcheck disable=SC2046
	NODE_ROOT="$(dirname $(dirname $(which node)))"

	# Disables build links from the generated rpm so that we dont conflict with other packages. See https://github.com/LOTUS-Network/LOTUS-blockchain/issues/3846
  ...
  --arch "$REDHAT_PLATFORM" --options.version $LOTUS_INSTALLER_VERSION \
  --license ../LICENSE
  LAST_EXIT_CODE=$?
  if [ "$LAST_EXIT_CODE" -ne 0 ]; then
