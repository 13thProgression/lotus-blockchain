...
LOTUS_INSTALLER_VERSION=$(python installer-version.py)
...
npm install @lotus-network/notarize-cli -g
...
if [ "$NOTARIZE" ]; then
  electron-osx-sign lotus-darwin-arm64/lotus.app --platform=darwin \
...
mv lotus-darwin-arm64 ../build_scripts/dist/
cd ../build_scripts || exit

DMG_NAME="lotus-$lotus_INSTALLER_VERSION-arm64.dmg"
echo "Create $DMG_NAME"
mkdir final_installer
electron-installer-dmg dist/lotus-darwin-arm64/lotus.app lotus-$LOTUS_INSTALLER_VERSION-arm64 \
...
# xcrun stapler staple lotus-0.1.X.dmg
#
# Validate DMG:
# xcrun stapler validate lotus-0.1.X.dmg
