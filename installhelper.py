...
def update_version():
    with open(f"{os.path.dirname(__file__)}/lotus-blockchain-gui/package.json") as f:
        data = json.load(f)

    version: str = "0.0"
    output = subprocess.run(["lotus", "version"], capture_output=True)
    if output.returncode == 0:
        version = str(output.stdout.strip(), "utf-8")

    data["version"] = make_semver(version)

    with open(f"{os.path.dirname(__file__)}/lotus-blockchain-gui/package.json", "w") as w:
