...
from lotus.wallet.derive_keys import master_sk_to_pool_sk, master_sk_to_wallet_sk
...
def create_all_ssl(root_path: Path):
    ...
    lotus_ca_crt, lotus_ca_key = get_lotus_ca_crt_key()
    generate_ssl_for_nodes(ssl_dir, lotus_ca_crt, lotus_ca_key, False, overwrite=False)
...
def init(create_certs: Optional[Path], root_path: Path, fix_ssl_permissions: bool = False):
    if create_certs is not None:
        if root_path.exists():
            ...
            if lotus_init(root_path, fix_ssl_permissions=fix_ssl_permissions) == 0 and root_path.exists():
                return init(create_certs, root_path, fix_ssl_permissions)

            print(f"** {root_path} was not created. Exiting **")
            return -1
    else:
        return lotus_init(root_path, fix_ssl_permissions=fix_ssl_permissions)


def lotus_version_number() -> Tuple[str, str, str, str]:
...
def lotus_init(
    ...
    """
    if os.environ.get("lotus_ROOT", None) is not None:
        print(
            f"warning, your lotus_ROOT is set to {os.environ['lotus_ROOT']}. "
            f"Please unset the environment variable and run lotus init again\n"
