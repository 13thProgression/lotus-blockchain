...
from lotus.cmds.init_funcs import check_keys, lotus_init
from lotus.cmds.passphrase_funcs import default_passphrase, using_default_passphrase
from lotus.daemon.keychain_server import KeychainServer, keychain_commands
from lotus.daemon.windows_signal import kill
from lotus.server.server import ssl_context_for_root, ssl_context_for_server
...
from lotus.util.service_groups import validate_service
from lotus.util.setproctitle import setproctitle
from lotus.util.ws_message import WsRpcMessage, create_payload, format_response
...
try:
    ...
    print("Error: Make sure to run . ./activate from the project folder before starting lotus.")
    quit()
...
service_plotter = "lotus plots create"
...
class WebSocketServer:
    ...
    async def exit(self) -> Dict[str, Any]:
        ...
        log.info("lotus daemon exiting in 5 seconds")
...
def launch_plotter(root_path: Path, service_name: str, service_array: List[str], id: str):
    # we need to pass on the possibly altered lotus_ROOT
...
def launch_service(root_path: Path, service_command) -> Tuple[subprocess.Popen, Path]:
    ...
    log.debug(f"Launching service with lotus_ROOT: {os.environ['lotus_ROOT']}")
...
async def async_run_daemon(root_path: Path, wait_for_unlock: bool = False) -> int:
    # When wait_for_unlock is true, we want to skip the check_keys() call in lotus_init
    # since it might be necessary to wait for the GUI to unlock the keyring first.
    lotus_init(root_path, should_check_keys=(not wait_for_unlock))
