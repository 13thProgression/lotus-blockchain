...
from lotus.protocols.shared_protocol import protocol_version
from lotus.server.introducer_peers import IntroducerPeers
from lotus.server.outbound_message import Message, NodeType
from lotus.server.ssl_context import private_ssl_paths, public_ssl_paths
from lotus.server.ws_connection import WSlotusConnection
from lotus.types.blockchain_format.sized_bytes import bytes32
from lotus.types.peer_info import PeerInfo
from lotus.util.errors import Err, ProtocolError
from lotus.util.ints import uint16
from lotus.util.network import is_localhost, is_in_network
from lotus.util.ssl import verify_ssl_certs_and_keys
...
class lotusServer:
    def __init__(
        ...
        lotus_ca_crt_key: Tuple[Path, Path],
        name: str = None,
        introducer_peers: Optional[IntroducerPeers] = None,
    ):
        # Keeps track of all connections to and from this node.
        logging.basicConfig(level=logging.DEBUG)
        self.all_connections: Dict[bytes32, WSlotusConnection] = {}
        self.tasks: Set[asyncio.Task] = set()

        self.connection_by_type: Dict[NodeType, Dict[bytes32, WSlotusConnection]] = {
            NodeType.FULL_NODE: {},
            NodeType.WALLET: {},
            NodeType.HARVESTER: {},
            NodeType.FARMER: {},
        ...
        self.lotus_ca_crt_path, self.lotus_ca_key_path = lotus_ca_crt_key
    ...
    async def garbage_collect_connections_task(self) -> None:
        ...
        while True:
            ...
            to_remove: List[WSlotusConnection] = []
    ...
    async def start_server(self, on_connect: Callable = None):
        ...
        if authenticate:
            ...
            ssl_context = ssl_context_for_server(
                self.lotus_ca_crt_path, self.lotus_ca_key_path, self.p2p_crt_path, self.p2p_key_path, log=self.log
            )
    ...
    async def incoming_connection(self, request):
        ...
        if peer_id == self.node_id:
            return ws
        connection: Optional[WSlotusConnection] = None
        try:
    ...
    async def connection_added(self, connection: WSlotusConnection, on_connect=None):
    ...
    async def start_client(
        ...
        if auth:
            ...
            ssl_context = ssl_context_for_client(
                self.lotus_ca_crt_path, self.lotus_ca_key_path, self.p2p_crt_path, self.p2p_key_path
            )
        session = None
        connection: Optional[WSlotusConnection] = None
        try:
    ...
    def connection_closed(self, connection: WSlotusConnection, ban_time: int):
    ...
    async def incoming_api_task(self) -> None:
        ...
        while True:
            ...
            async def api_call(full_message: Message, connection: WSlotusConnection, task_id):
    ...
    def get_outgoing_connections(self) -> List[WSlotusConnection]:
        result = []
        for _, connection in self.all_connections.items():
    ...
    def get_full_node_outgoing_connections(self) -> List[WSlotusConnection]:
        result = []
    ...
    def get_full_node_connections(self) -> List[WSlotusConnection]:
        return list(self.connection_by_type[NodeType.FULL_NODE].values())

    def get_connections(self, node_type: Optional[NodeType] = None) -> List[WSlotusConnection]:
        result = []
        for _, connection in self.all_connections.items():
    ...
    async def get_peer_info(self) -> Optional[PeerInfo]:
        ...
        # Use lotus's service first.
        try:
            timeout = ClientTimeout(total=15)
            async with ClientSession(timeout=timeout) as session:
                async with session.get("https://ip.lotus.net/") as resp:
                    if resp.status == 200:
    ...
    def is_trusted_peer(self, peer: WSlotusConnection, trusted_peers: Dict) -> bool:
