...
from lotus.consensus.cost_calculator import calculate_cost_of_program, NPCResult
...
from lotus.wallet.puzzles.puzzle_utils import (
...
from lotus.wallet.secret_key_store import SecretKeyStore
from lotus.wallet.sign_coin_spends import sign_coin_spends
...
class Wallet:
    ...
    async def create_spend_bundle_relative_lotus(self, lotus_amount: int, exclude: List[Coin]) -> SpendBundle:
        list_of_solutions = []
        utxos = None
        ...
        if lotus_amount < 0:
            utxos = await self.select_coins(abs(lotus_amount), exclude)
        else:
        ...
        lotus_amount = spend_value + lotus_amount
        ...
        for coin in utxos:
            ...
            if output_created is None:
                ...
                primaries = [{"puzzlehash": newpuzhash, "amount": lotus_amount}]
