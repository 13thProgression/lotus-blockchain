...
from lotus.pools.pool_puzzles import SINGLETON_LAUNCHER_HASH
from lotus.protocols import wallet_protocol
from lotus.protocols.full_node_protocol import RequestProofOfWeight, RespondProofOfWeight
from lotus.protocols.protocol_message_types import ProtocolMessageTypes
from lotus.protocols.wallet_protocol import (
...
from lotus.server.node_discovery import WalletPeers
...
from lotus.util.ints import uint32, uint128
from lotus.util.keychain import Keychain
from lotus.util.lru_cache import LRUCache
from lotus.util.merkle_set import MerkleSet, confirm_included_already_hashed, confirm_not_included_already_hashed
from lotus.util.path import mkdir, path_from_root
from lotus.wallet.block_record import HeaderBlockRecord
from lotus.wallet.derivation_record import DerivationRecord
from lotus.wallet.settings.settings_objects import BackupInitialized
from lotus.wallet.transaction_record import TransactionRecord
from lotus.wallet.util.backup_utils import open_backup_file
from lotus.wallet.util.wallet_types import WalletType
from lotus.wallet.wallet_action import WalletAction
from lotus.wallet.wallet_blockchain import ReceiveBlockResult
...
class WalletNode:
    ...
    async def on_connect(self, peer: WSlotusConnection):
        if self.wallet_state_manager is None or self.backup_initialized is False:
            return None
    ...
    async def complete_blocks(self, header_blocks: List[HeaderBlock], peer: WSlotusConnection):
    ...
    async def new_peak_wallet(self, peak: wallet_protocol.NewPeakWallet, peer: WSlotusConnection):
        if self.wallet_state_manager is None:
            return
    ...
    async def _sync(self) -> None:
        ...
        peers: List[WSlotusConnection] = self.server.get_full_node_connections()
        if len(peers) == 0:
    ...
    async def get_additions(
        self, peer: WSlotusConnection, block_i, additions: Optional[List[bytes32]], get_all_additions: bool = False
    ) -> Optional[List[Coin]]:
    ...
    async def get_removals(
        self, peer: WSlotusConnection, block_i, additions, removals, request_all_removals=False
    ) -> Optional[List[Coin]]:
        assert self.wallet_state_manager is not None
