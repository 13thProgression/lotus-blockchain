...
from lotus.daemon.keychain_proxy import connect_to_keychain_and_validate, wrap_local_keychain
...
from lotus.full_node.mempool_check_conditions import GENERATOR_MOD
from lotus.plotting.create_plots import create_plots, PlotKeys
...
from lotus.consensus.coinbase import create_puzzlehash_for_pk, create_farmer_coin, create_pool_coin
...
from lotus.consensus.cost_calculator import NPCResult, calculate_cost_of_program
...
from lotus.plotting.util import PlotInfo, PlotsRefreshParameter, PlotRefreshResult, parse_plot_info
...
from lotus.types.blockchain_format.reward_chain_block import RewardChainBlockUnfinished
from lotus.types.blockchain_format.sized_bytes import bytes32
from lotus.types.blockchain_format.slots import (
...
from lotus.util.condition_tools import ConditionOpcode, conditions_by_opcode
from lotus.util.config import load_config, save_config
...
class BlockTools:
    ...
    def __init__(
        ...
        create_default_lotus_config(root_path)
...
def get_plot_dir() -> Path:
    cache_path = Path(os.path.expanduser(os.getenv("lotus_ROOT", "~/.lotus/"))) / "test-plots"
    mkdir(cache_path)
    return cache_path
